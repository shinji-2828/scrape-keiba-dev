...
url = "https://db.netkeiba.com/race/202003010302"
r=requests.get(url)
...
import csv
file_path = '/Users/dev.csv'
with open(file_path, 'w', newline='',encoding="SHIFT-JIS") as f:#自分でパスを設定してください、ファイル名はXXXX.csvの形です
    csv.writer(f).writerows(houseInfo)
    csv.writer(f).writerows(raceInfo)
    csv.writer(f).writerows(payBackInfo)

# import pandas
# pandas.read_csv(file_path,encoding="SHIFT-JIS",header=None)