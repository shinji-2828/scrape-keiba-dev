...
for w in range(len(l)):
    for z in range(7):#開催、6まで十分だけど保険で7
        for y in range(14):#日、12まで十分だけど保険で14
            if y<9:
            ...
            for x in range(12):#レース12
                if x<9:
                    ...
                    url=url1+str(x+1)
                r=requests.get(url)
                time.sleep(0.5)#サーバーの負荷を減らすため0.5秒待機する
                soup = BeautifulSoup(r.content.decode("euc-jp", "ignore"), "html.parser",from_encoding='utf-8')#バグ対策でdecode
                soup_span = soup.find_all("span")
                allnum=(len(soup_span)-9)/3#馬の数
                if allnum==-1/3 or allnum == 0:#urlにデータがあるか判定
                    yBreakCounter+=1
                    continue
                allnum=int(allnum)
                #馬の情報
                ...
                #レースの情報
                sur=str(soup_span[8]).split("/")[0].split(">")[1][0]
                rou=str(soup_span[8]).split("/")[0].split(">")[1][1]
                dis=str(soup_span[8]).split("/")[0].split(">")[1].split("m")[0][-4:]
                con=str(soup_span[8]).split("/")[2].split(":")[1][1]
                wed=str(soup_span[8]).split("/")[1].split(":")[1][1]
...
import csv
with open('/Users/'+year+'.csv', 'w', newline='',encoding="SHIFT-JIS") as f:
